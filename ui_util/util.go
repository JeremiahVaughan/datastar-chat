...
type TemplateLoader struct {
	templatesBaseDirName      string // included in every template
	templatesBaseDir          string
	templatesOverridesDirName string // only added if each file listed individually in the HtmlTemplate struct
...
func NewTemplateLoader(
	templatesBaseDirName string,
	templatesOverridesDirName string,
	htmlTemplates []HtmlTemplate,
	localMode bool,
) (*TemplateLoader, error) {
	tl := TemplateLoader{
		templatesBaseDirName:      templatesBaseDirName,
		templatesOverridesDirName: templatesOverridesDirName,
		htmlTemplates:             htmlTemplates,
		templateMap:               make(map[string]*template.Template),
	}
	execPath, err := os.Executable()
	if err != nil {
		return nil, fmt.Errorf("error, when retrieving executable path. Error: %v", err)
	}
	executableDir := filepath.Dir(execPath)
	tl.templatesBaseDir = fmt.Sprintf("%s/%s", executableDir, tl.templatesBaseDirName)
	tl.templatesOverridesDir = fmt.Sprintf("%s/%s", executableDir, tl.templatesOverridesDirName)
	err = tl.parseTemplates()
	if err != nil {
		log.Fatalf("error, when parseTemplates() for main(). Error: %v", err)
	}
	if localMode {
